...
const AccordNew = ({ data, index }) => {
  ...
  return (
    <>
      <div className="flex flex-col md:flex-row justify-between items-center gap-7 w-full border p-5 md:px-7 md:py-10 rounded-xl shadow-lg transition-all duration-300">
        ...
        <div className="md:flex-1 text-left">
          ...
          <p className="text-gray-600 font-medium text-sm lg:text-xl pr-8">{data.desc}</p>
        </div>
      ...
      {showData && (
        <div className="mx-auto md:w-[85%] p-5 space-y-10">
          {data?.roles.map((role, idx) => (
            <div
              key={idx}
              className="shadow-lg flex flex-col md:flex-row justify-between items-center gap-3 md:gap-7 w-full border px-7 py-10 rounded-xl transition-all duration-300"
            >
